  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-2xl bg-carvao-50 px-4 py-3 ring-1 ring-carvao-100 dark:bg-carvao-900/40 dark:ring-carvao-700/50">
        ...
        <span className="ml-auto text-[11px] font-bold uppercase tracking-wide text-brand-600 dark:text-brand-400">
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        >
          ...
          <div className="min-w-0">
            <p className="font-display text-[15px] font-bold text-carvao-900 dark:text-white">
              {nome}
            </p>
            {obs && (
              <p className="truncate text-[11px] text-carvao-400" title={obs}>
                {obs.slice(0, 80)}{obs.length > 80 ? '…' : ''}
              </p>
            )}
          </div>
          <span className="ml-auto text-[12px] font-semibold text-carvao-400">
            {confirmados}/{total}
          </span>
          <Icone
        ...
        {nome !== 'Sem fornecedor' && (
          <button
            ...
            className="flex h-9 items-center gap-1.5 rounded-xl bg-[#25D366]/10 px-3 text-[12px] font-bold text-[#128C7E] ring-1 ring-[#25D366]/30 transition hover:bg-[#25D366]/20 dark:text-[#25D366]"
          >
                ...
                <div className="flex items-center gap-1">
                  {podeEditar ? (
                    <input
                      ...
                      className="h-8 w-14 rounded-lg border border-carvao-200 bg-white px-1.5 text-center text-[12px] font-bold tabular-nums dark:border-carvao-600 dark:bg-carvao-900"
                    />
                  ) : (
                    <strong className="tabular-nums text-[13px]">{formatarQtd(qtdFinal)}</strong>
                  )}
                  <span className="text-[11px] text-carvao-400">{unid}</span>
                </div>
                ...
                <div className="w-[90px]">
                  {podeEditar ? (
                    <input
                      ...
                      className="h-8 w-full rounded-lg border border-carvao-200 bg-white px-1 text-[11px] dark:border-carvao-600 dark:bg-carvao-900 dark:[color-scheme:dark]"
                    />
                  ) : p.previsao ? (
                    <span className="text-[12px] font-semibold tabular-nums text-carvao-700 dark:text-areia-200">
                      {ddmm(p.previsao)}
                    </span>
                  ) : (
                    <span className="text-carvao-300 text-[12px]">—</span>
                  )}
                </div>
